# -*- coding: utf-8 -*-
"""
Created on Tue Nov 23 09:57:25 2021
...
if __name__ == '__main__':
    ...
    # C=[]
    # D=[]
    # print('Pour les variable nul on ecrit un 0. par ex, x+5x2+x4 donc on a la valeur du coefficient de variable x3= 0')
    
    # prb=str(input("Veuillez entrer le type de problème (max ou min) : "))
    # const=int(input("Combien y a-t-il de contraintes ? "))
    
    # for i in range(const):
    #     contrain = [int(x) for x in input(f'Entrer la liste de la valeur du coefficient de variable dans contrainte numero {i+1} (ex: 1 1 4): ').split()]
    #     A.append(contrain)
    #     t=[int(x) for x in input(f'Type de la contrainte numero {i+1} (1 est <=, 0 est =, -1 est >=): ').split()]
    #     D.append(t)
    #     r=[int(x) for x in input(f'Entrer la valeur du résultat dans contrainte numero {i+1} (ex: pour 2x1 + x2 ≤ 70 donc on écrit 70 ): ').split()]
    #     B.append(r)
    # var=int(input("Combien y a-t-il de variable ? "))
    # for j in range(var):
    #     c=[int(x) for x in input(f'Entrer la valeur de variable numéro {j+1} dans fonction objective (ex: 5):').split()]
    #     C.append(c)
        
    # (z, x) = simplex(prb, np.array(A),
    #                 np.array(B),
    #                 np.array(C),
    #                 np.array(D),
    #           10000)
...
# x1, x2 ≥ 0
    ...
    # (z, x) = simplex('max', np.array([[1, 1, 0], [1, 0, 1], [0, 1, 1]]),
    #                     np.array([[20], [5], [10]]),
    #                     np.array([[1], [-1], [3]]),
    #                     np.array([[1], [0], [-1]]),
    #              10000)
